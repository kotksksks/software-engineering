...
## Выводы
В данном задании мы явно типизировали список и кортеж. Из строки можно создать список, но строку нельзя преобразовать в кортеж так как они являются неизменяемыми.
...
## Выводы
В данном коде мы пытаемся удалить элемент из кортежа, но кортежи являются неизменяемыми, поэтому  нельзя напрямую удалить элемент из кортежа. Но преобзазуя кортеж в список можно удалить элемент, а затем преобразовать список обратно в кортеж.
...
## Выводы
В данном задании мы знакомимся с типом данных словарь, создаём пару ключ-значение, где ключом является число, а значением увеличивающееся значение при каждом вхождении слова. Продолжаем работу со словарём сортируя значения (частоту) в порядке возрастания. Далее сортируем ключи в порядке возрастания и выводим три самые встречающиеся пары. Используя анонимную функцию лямбда можно просто отсортировать словарь не по ключам, а по значениям.
...
## Выводы
В данном задании реализован поиск по элементам кортежа, срезы реализованы точно так же, как для прочих упорядоченных коллекций.
...
## Выводы
В данном задании реализован генератор кортежа, что показывает, что кортеж также может быть создан с помощью генератора.

## Общие выводы по теме
В данной теме мы ознакомились с такими типами данных как словари и кортежи. Словари хранят пары ключ-значение, где ключ является неизменяемым и уникальным, а значение изменяемое и может повторяться. Кортежи — упорядоченные коллекции элементов, которые могут быть любого типа. Кортежи используются для создания неизменяемых упорядоченных коллекций объектов.
